(() => {
  ...
  const LIMIT = 9, POINT = 10, T = 60; // 문항수/점수/제한시간(초)
  const ORANGE = '#ff6b00';

  // ====== 유틸 ======
  const log = (...args) => console.log(NS, ...args);
  const warn = (...args) => console.warn(NS, ...args);
  const err = (...args) => console.error(NS, ...args);
  const onReady = (fn) => {
  ...
  const ensureHost = () => {
    ...
    if (!host) {
      ...
      log('mount host was missing; created and appended to body');
    }
    return host;
  };
  const shadowSupported = () => !!(Element.prototype.attachShadow);
  ...
  const ALL_QUESTIONS = [
    ...
    {type:'객관식', q:'다음의 게임 주인공들 중 머리색이 다른 한 명을 고르시오.', options:['A. 병원x생활-하미나','B. 푸른 달빛 속에서-은랑','C. 아르티스 임팩트-아카네','D. 서큐하트-이에','E. 스칼렛 크로스-엘리엇'], answer:'B'},
    {type:'객관식', q:'다음 공포 게임들 중 게임의 이름이 올바르지 않은 것을 고르시오.', options:['A. 고독방송','B. 공포의 섹시 물귀신','C. 후즈 앳 더 도어','D. 언더월드','E. SAEKO'], answer:'D'},
    {type:'객관식', q:'다음 중 도트로 디자인된 게임이 아닌 것을 고르시오.', options:['A. 캐틀 컨트리','B. 리듬 마녀: 죽음과 함께 춤을','C. 폭풍의 메이드','D. 리서치 스토리','E. 아르티스 임팩트'], answer:'B'},
    {type:'객관식', q:'9월에 진행 되었던 구구 후기 이벤트에서 "후기 작성" 대상 게임이 아닌 것을 고르시오.', options:['A. 다크 데이티 2','B. 푸른 달빛 속에서','C. 디트로이트: 비컴 휴먼','D. 사무라이 쇼다운 II','E. 하모닉스'], answer:'D'},
    {type:'객관식', q:"비주얼 노벨 명가 '테일즈샵'의 감성 로맨스 신작, <사랑 한 잔 말아주세요!>이 12월 23일 정식출시 예정입니다. 게임 속에서 주인공은 어느 술집의 바텐더로 일하게 되는데, 다음 중 '술'과 관련이 없는 게일을 고르시오.", options:['A. 프렌치커넥션','B. 내 일상에 스며든 비늘','C. 헌드레드 데이즈','D. 술도 사랑도 어려운 아가씨','E. 과몰입금지2: 여름포차'], answer:'B'},
    {type:'객관식', q:'<골목길 : 귀흔>의 주인공 "소연"의 가문과 관련이 깊은 단어를 고르시오.', options:['A. 염화','B. 연화','C. 이화','D. 예화','E. 유화'], answer:'B'},
    {type:'객관식', q:'<골목길 : 귀흔>의 지하 불상 퍼즐과 관련이 없는 것을 고르시오.', options:['A. 나무','B. 불','C. 빛','D. 흙','E. 물'], answer:'C'},
    {type:'객관식', q:'<고독방송>의 주인공 유나는 스트리머입니다. 그녀의 팬네임을 고르시오.', options:['A. 유나닷치','B. 유나도치','C. 유나하치','D. 유나둠치','E. 유나해치'], answer:'A'},
    {type:'객관식', q:'<폭풍의 메이드>와 콜라보 DLC의 출시가 빠른 순서대로 나열된 것을 고르시오. [보기] 1.블랙 하트 2.썸썸 편의점 3.서큐하트 4.러브 스티치 5.프렌치 커넥션 6.기적의 분식집', options:['A. 3>4>2>6>1>5','B. 2>6>1>3>5>4','C. 1>3>4>2>6>5','D. 5>3>4>6>2>1'], answer:'A'},
    {type:'객관식', q:"다음 중 '스토브 한글화'로 출시한 게임이 아닌 것을 고르시오.", options:['A. 하요와 잡화점','B. 아르티스 임팩트','C. 슈퍼 판타지 킹덤','D. 요그 소토스의 정원','E. 보더 타운'], answer:'A'},
    {type:'객관식', q:"다음 중 지금까지 초특가 할인 이벤트 '빅딜'을 진행한 적 없는 게임을 고르시오.", options:['A. 천사의 제국4','B. 서큐하트','C. 칼리스','D. 스플래셔','E. 스웜프 키퍼'], answer:'E'},
    {type:'주관식', q:'한국의 주택가와 길거리를 사실적으로 묘사하여 큰 화제가 된 아래 공포 게임의 이름 중 빈칸에 해당하는 부제를 적으시오.[보기] <골목길 : OO>(※한국어 명칭 띄어쓰기 없이 기입)', answer:'귀흔'},
    {type:'주관식', q:'<골목길 : 귀흔>에서 귀신의 장난을 뜻하는 단어를 적으시오.', answer:'허깨비'},
    {type:'주관식', q:'서부 개척지의 모험과 낭만이 가득한 시뮬레이션 게임으로, 10월 31일 스토브 한글화로 출시된 이 게임의 이름을 적으시오.(※한국어 명칭 띄어쓰기 없이 기입)', answer:'캐틀컨트리'},
    {type:'주관식', q:'<고독방송> 속 게임 대결 합방에서 유나를 0대 33으로 완패시켰던 요리 스트리머의 이름을 적으시오.(※한국어 명칭 띄어쓰기 없이 기입)', answer:'귀신든램쥐'},
    {type:'주관식', q:'<고독방송>에서 유나는 게임 대결 합방에서 패배한 벌칙으로 흉가 체험 라이브 방송을 하게 됩니다. 흉가 체험을 위해 찾아간 장소의 이름을 적으시오.(※한국어 명칭 띄어쓰기 없이 기입)', answer:'신처럼정신병원'},
    {type:'주관식', q:'<클레르 옵스퀴르: 33 원정대>에서 일 년에 한번씩 잠에서 깨어나 거석 위에 저주받은 숫자를 적는 "이것"의 이름을 적으시오.(※한국어 명칭 띄어쓰기 없이 기입)', answer:'페인트리스'},
    {type:'주관식', q:'<아르티스 임팩트>의 주인공 아카네가 속한 AI 저항조직의 이름을 적으시오.(※한국어 명칭 띄어쓰기 없이 기입)', answer:'리트'},
    {type:'주관식', q:"2022년부터 일 년에 한 번, 스마일게이트 주체로 진행되었던 인디게임 축제 '버닝비버'가 2025을 맞이하여 새로운 이름으로 개최됩니다. 인디게임 창작자들의 열정과 창의성을 공유하는 이 축제의 새로운 이름을 적으시오.(※한국어 명칭 띄어쓰기 없이 기입)", answer:'비버롹스'}
  ];
  ...
  .muted{ color:#0B0B0D; }
  .start-actions{ display:flex; gap:12px; justify-content:center; margin-top:24px; flex-wrap:wrap; }
  ...
  // ====== 앱 로직 ======
  function mountApp(rootHost, useShadow) {
    log('mounting', { useShadow });

    // 재마운트 방지
    if (rootHost.dataset.stoveQuizMounted === '1') {
      warn('already mounted; skip');
      return;
    }
    rootHost.dataset.stoveQuizMounted = '1';

    // 컨테이너 생성 (Shadow DOM or Light DOM)
    let container, $;
    if (useShadow) {
      ...
      shadowRoot.append(style, tpl.content.cloneNode(true));
      ...
      rootHost.innerHTML = '';
      const style = document.createElement('style');
      style.textContent = styleText.replace(':host { all: initial; }', '/* no shadow host */');
      const wrap = document.createElement('div');
      wrap.innerHTML = htmlTpl;
      rootHost.append(style, wrap);
      $ = (sel) => rootHost.querySelector(sel);
      container = rootHost;
      warn('Shadow DOM not supported; using light DOM fallback');
    }
    ...
    const elStart = $('#start'),
          ...
          elResult = $('#result'),
          elProgress = $('#progress'),
          elTimeLabel = $('#timeLabel'),
          elQuestion = $('#question'),
          elOptions = $('#options'),
          elInputWrap = $('#inputWrap'),
          elAnswer = $('#answerInput'),
          elSubmit = $('#submitBtn'),
          elScoreText = $('#scoreText'),
          elTimeText = $('#timeText'),
          elDetail = $('#detailTimes'),
          elCongrats = $('#congrats');

    // 이벤트 바인딩
    $('#btnYes')?.addEventListener('click', startQuiz);
    $('#btnNo')?.addEventListener('click', () => {
      alert('다음에 다시 도전해 주세요!');
    });
    ...
    function startQuiz() {
      ...
      showQuestion();
      startTimer();
      log('quiz started', { total: quizData.length, limitSec: T });
    }
    ...
    function showQuestion() {
      ...
      if (q.type === '객관식') {
        ...
        q.options.forEach((o) => {
          ...
          btn.textContent = o;
          // 옵션 첫 글자(예: 'A')로 정답 판정
          const letter = (o.match(/^[A-Ea-e]/)?.[0] || o[0] || '').toUpperCase();
    ...
    function showResult() {
      ...
      log('quiz finished', { score, perTimes, total });
    }
  ...
  onReady(() => {
    try {
      ...
      const useShadow = shadowSupported();
      mountApp(host, useShadow);
    } catch (e) {
      err('mount failed:', e);
      alert('퀴즈 로딩 중 오류가 발생했습니다. 콘솔 로그를 확인해 주세요.');
    }
  });
